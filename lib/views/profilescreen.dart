  ...
  Future<void> _verifyIdentity() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('https://api-para-verificar-identidad/$numeroidentidad'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'found_person') {
          // Persona encontrada, pedir contraseña para crear el usuario
          setState(() {
  ...
  Future<void> _registerUser() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://tu-api.com/api/create-user'),
        headers: {
          'Content-Type': 'application/json',
        },
        body: jsonEncode({
          'identity_number': numeroidentidad,
          'password': password,
        }),
      );
